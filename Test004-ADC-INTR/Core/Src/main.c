...
/* USER CODE BEGIN 0 */
int val1, val2, ch = 0;
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)	//Sampling complete, **Channel management required
 {
    if(ch == 0)
      {
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim3);	//Timer InterrputS
  HAL_ADC_Start_IT(&hadc1);		//ADC Interrupt
