...
int main(void)
{
  ...
  while (1)
    ...
    /* USER CODE BEGIN 3 */
      HAL_ADC_Start (&hadc1);		      //1 time start
...
/*      if(!(HAL_GPIO_ReadPin (Zaxis_GPIO_Port, Zaxis_Pin)))
	{
	  printf ("Joystick value - X, Y : pushed(%d, %d)\r\n",val1/51, val2/171);
	}
      else printf ("Joystick value - X, Y : (%d, %d)\r\n",val1/51, val2/171);*/
      ...
      if(!(HAL_GPIO_ReadPin (Zaxis_GPIO_Port, Zaxis_Pin)))
	{
      ...
      else printf ("\033[%d;%dH", val2/171,val1/51);
    }
  /* USER CODE END 3 */
}
